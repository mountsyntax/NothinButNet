...
content: Container {
            ...
            onTouch: {
                console.log("stats.qml: onTouch");
                if (event.isDown())
                {
                    console.log("stats.qml: onTouch: event.isDown");
                    Utils.startTouch(event.windowX, event.windowY);
                }
                else if (event.isUp()) {
                    console.log("stats.qml: onTouch: event.isUp");
                    Utils.endTouch();
                }
                else if (event.isMove()) {
                    console.log("stats.qml: event.isMove: " + "x: " + event.windowX + " y: " + event.windowY);
                    Utils.tapped = false;
                    Utils.moveTouch(event.windowX, event.windowY);
                        

                    if (Utils.isSwipeRight())
                    {
                        console.log("Utils.isSwipeRight");
                        if(Utils.rightSwiped == true)
                        {
                            console.log("Utils.rightSwiped");
                        }
                        else
                        {
                            Utils.rightSwiped = true;
                            console.log("cs.goBack");
                            cs.goBack();
                        }
                    }
                }
